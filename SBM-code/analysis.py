...
def compare_result_graph_vs_array_structure(X, y, n_clusters, eps, pn=25):
    sp.plot('Synthetic dataset (Sim1) ground truth', X, y, marker='o', alpha=0.5)

    kmeans = KMeans(n_clusters=n_clusters, random_state=0).fit(X)
    sp.plot('KMeans on UO', X, kmeans.labels_, marker='o')
